...
docker build \
  ...
  -t rootventures/pytorch

docker build \
  -o type=registry,oci-mediatypes=true,compression=estargz,force-compression=true,push=true \
  --builder builder \
  -f dockerfiles/Dockerfile.train . \
  -t rootventures/train-dreambooth

docker build \
  -o type=registry,oci-mediatypes=true,compression=estargz,force-compression=true,push=true \
  --builder builder \
  -f dockerfiles/Dockerfile.sagemaker . \
  -t rootventures/train-dreambooth-sagemaker

docker tag rootventures/train-dreambooth-sagemaker 630351220487.dkr.ecr.us-west-2.amazonaws.com/train-dreambooth-sagemaker:latest
